...
public:
	...
	void RemoveFood(int value);
	void Expand(EltType value);
...
// Drop a given element
// Added by Eden Forbes
template<class EltType>
void TVector<EltType>::RemoveItem(int value)
{
	for (int i = value; i < ub; i+=1){
		Vector[i] = Vector[i+1];
	}
}

template<class EltType>
void TVector<EltType>::RemoveFood(int value)
{
	for (int i = 0; i < ub; i+=1){
		if(Vector[i] == value){
			for (int j = i; j < ub; j+=1){
				Vector[j] = Vector[j+1];
			}
			break;
...
// Added by Eden Forbes
template<class EltType>
void TVector<EltType>::Expand(EltType value)
{
	ub += 1;
	printf("ub = %d\n", ub);
	Vector[ub] = value;
